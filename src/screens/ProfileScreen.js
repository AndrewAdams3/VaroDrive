...
export default ProfileScreen = ({navigation}) => {
    ...
    useEffect(()=>{
        ...
        if(profilePic){
            ...
            path = path.join('/')
            setprofilepic(constants.ip + '/' + path)
          }
        else{
    ...
    const removeToken = async () => {
        try {
            ...
            var url = constants.ip + '/data/users/logout';
      ...
      <View style={styles.container}>
        ...
        <View style={styles.mainView}>
          <View style={{ flex: 2, alignContent: 'flex-start', justifyContent: 'flex-start' }}>
            <FastImage
              onError={() => {
                  setprofilepic(constants.ip + '/' + "file/uploads/profilePics/default.png")
              }}
              style={styles.profilePic}
              source={{ uri: constants.ip + '/' + profilePic }}
              resizeMode="cover"
            />
          </View>
